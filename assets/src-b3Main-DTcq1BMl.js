...
`,"Zirze"),WA.player.state.currentQuest="quest21";const s=[],n=[];for(let e=4;e<=15;e++)for(let a=71;a<=89;a++)s.push({x:e,y:a,tile:"green",layer:"green"}),n.push({x:e,y:a,tile:null,layer:"red"});WA.room.setTiles(s),WA.room.setTiles(n),l("bodul_3",10);const t=await WA.nav.getCoWebSites();for(const e of t)e.close()}}),WA.player.state.onVariableChange("m3terminal2").subscribe(async r=>{if(WA.player.state.module3="2",r==="correct"){WA.player.state.currentQuest="quest25";const s=[],n=[];for(let e=4;e<=15;e++)for(let a=47;a<=70;a++)s.push({x:e,y:a,tile:"green",layer:"green"}),n.push({x:e,y:a,tile:null,layer:"red"});WA.room.setTiles(s),WA.room.setTiles(n),l("bodul_3",10);const t=await WA.nav.getCoWebSites();for(const e of t)e.close()}})});WA.player.state.onVariableChange("module3").subscribe(r=>{if(r==="2"){const s=[],n=[];for(let t=4;t<=15;t++)for(let e=71;e<=89;e++)s.push({x:t,y:e,tile:"green",layer:"green"}),n.push({x:t,y:e,tile:null,layer:"red"});WA.room.setTiles(s),WA.room.setTiles(n),WA.chat.sendChatMessage(`##### 🔍 Weitere Wortschnipsel gefunden!   
...
`,"Zirze")}});WA.player.state.onVariableChange("currentQuest").subscribe(r=>{r==="quest26"&&WA.chat.sendChatMessage("Wow, das ging schnell! Du hast beide Räume gemeistert. Ich hoffe du kannst dich noch an alle Wortschnipsel erinnern. Diese musst du nun in richtiger Reihenfolge im Sicherheitsterminal eingeben. Falls du Hilfe brauchst, frag doch deine Kolleg*innen, ob ihr diese Aufgabe zusammen lösen könnt. Ich darf nicht zu viel verraten, aber eine gezielte Recherche könnte durchaus hilfreich sein. Wenn du oder ihr es schafft, können wir Lord Modrevolt endlich aus unserem System entfernen und unsere Sicherheitseinstellungen des Kondensatoriums wieder herstellen. ","Zirze")});WA.player.state.onVariableChange("Textarten").subscribe(r=>{r==="solved"&&(l("bodul_3",10),console.log('Variable "Textarten" solved. Level up, +10XP'),WA.player.state.currentQuest="quest18",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.player.state.onVariableChange("AllgemeineRegeln").subscribe(r=>{r==="solved"&&(l("bodul_3",10),console.log('Variable "AllgemeineRegeln" solved. Level up, +10XP'),WA.player.state.currentQuest="quest19",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.player.state.onVariableChange("Sprache").subscribe(r=>{r==="solved"&&(l("bodul_3",10),console.log('Variable "Sprache" solved. Level up, +10XP'),WA.player.state.currentQuest="quest20",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.player.state.onVariableChange("Zitieren").subscribe(r=>{r==="solved"&&(l("bodul_3",10),console.log('Variable "Zitieren" solved. Level up, +10XP'),WA.player.state.currentQuest="quest22",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.player.state.onVariableChange("Literaturverzeichnis").subscribe(r=>{r==="solved"&&(l("bodul_3",10),console.log('Variable "Literaturverzeichnis" solved. Level up, +10XP'),WA.player.state.currentQuest="quest23",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.player.state.onVariableChange("Literaturverwaltung").subscribe(r=>{r==="solved"&&(l("bodul_3",10),console.log('Variable "Literaturverwaltung" solved. Level up, +10XP'),WA.player.state.currentQuest="quest24",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.player.state.onVariableChange("Abschlussquiz3").subscribe(r=>{r==="solved"&&(l("bodul_3",10),WA.room.hideLayer("blockPortals"),console.log('Variable "finalQuizThree" solved. Level up, +10XP'),WA.player.state.currentQuest="quest26",setTimeout(()=>{try{WA.chat.close()}catch(s){console.error("Failed to close chat:",s)}},6e4))});WA.onInit().then(async()=>{if(WA.player.name.toLowerCase()==="bot"||WA.player.tags.includes("bot")){WA.player.setOutlineColor(147,51,234),await m();return}await f(),v(),T()});
//# sourceMappingURL=src-b3Main-DTcq1BMl.js.map
